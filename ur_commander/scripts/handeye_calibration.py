...
class HandEyeCalibrationSample:
    ...
    def euler_to_quad(self):
        ...
        print("\nThe entered pose is:")
        print(
            self.pose_x, self.pose_y, self.pose_z, self.pose_r1, self.pose_r2, self.pose_r3, sep=","
        )
        print("The converted pose (Euler angles --> quaternions) is: ")
        print(self.pose_x, self.pose_y, self.pose_z, quad_w, quad_x, quad_y, quad_z, sep=",")
        return HandEyeTransformation(
            self.pose_x, self.pose_y, self.pose_z, quad_w, quad_x, quad_y, quad_z
        )
    ...
    def calibrate(self):
        ...
        print("During the hand-eye calibration, please make sure you enter enough robot poses")
        ...
        while not calibrate:
            ...
            if command == "P":  # Obtain the original 2D image
                ...
                show_and_save_image(
                    frame_2d.get_color_image(),
                    file_name="Original2DImage_" + str(self.pose_index) + ".png",
                    window_name="Original 2D Image",
                )
            elif command == "T":  # Obtain the 2D image with feature point recognition results.
                frame_3d = Frame3D()
                show_error(self.camera.capture_3d(frame_3d))
                show_and_save_image(
                    frame_3d.get_depth_map(),
                    file_name="DepthMap_" + str(self.pose_index) + ".tiff",
                    window_name="Depth",
                )
                color_image = Color2DImage()
                show_error(self.calibration.test_recognition(self.camera, color_image))
                show_and_save_image(
                    color_image,
                    file_name="FeatureRecognitionResultForTest_" + str(self.pose_index) + ".png",
                    window_name="Feature Recognition Result For Test",
                )
            elif (
                command == "A"
            ):  # Input the current robot pose. The unit of the translational component is mm, and the unit of the Euler angles is degrees.
                ...
                show_and_save_image(
                    color_image,
                    file_name="FeatureRecognitionResult_" + str(self.pose_index) + ".png",
    ...
    def main(self):
        ...
        show_error(
            self.calibration.initialize_calibration(
